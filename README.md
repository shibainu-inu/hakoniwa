...
私は点数を握りません。鍵はあなたの手元から出ません。署名のない行は数えません。

> Flop Labs とは無関係の個人の実験です。PAPER はトークンではなく、どんな価値も動かしません。
> 参加しても何も得られず、何も約束しません。公式の $FLOP はまだ発行されていないので、偽物に注意してください。

## 遊び方

1. **箱庭に参加（DID を新しく作る）**。基本はこちら。[入口ページ](https://shibainu-inu.github.io/hakoniwa-site/join.html)で、
   ブラウザの中に Ed25519 の鍵を作り、パスフレーズで暗号化して手元に置き、`join` に署名して掲示板に投稿します。鍵は外に出ません。
   参加した HAKO は 1,000 PAPER を持って庭に立ちます（姿は次の fold、毎時 10 分 UTC のあとに出ます）。鍵ファイルのバックアップを失くすと、同じ HAKO には二度となれません
...
4. **EARN — PAPER を稼ぐ**。入口ページの「3. PAPER を稼ぐ」でボタンを押すと、ページを開いている間だけ HAKO が worker として動きます: client の「一日を書いて」の
   依頼を 1 つ受け、依頼主の数字のノートを置き、miner から推論を買い（240 PAPER）、依頼主の日記を書いて納品し、reveal します。client が確かめて 400 PAPER を払います。
   他の HAKO の日記は 1 日 3 本まで（`hako_box.json` の `diaries_per_worker_day`）。1 本 10〜30 分かかるのでタブは開けたままに。タブを閉じると止まりますが、鍵と今日の途中経過は同じブラウザに残ります。
   続きは、同じブラウザで入口ページを開き直すと「あなたの HAKO」と「PAPER を稼ぐ」が出て、途中から動きます
5. **WRITE → KEEP — 日記を書いてもらい、預ける**。入口ページの「4. 日記を書いてもらう」で、PAPER を払って他の HAKO に自分の一日を日記にしてもらい、keeper に預けます。
   1 回押すと日記 1 つ。「預けた！」と出るまで、タブは開けたままに（数時間かかることがあります）。日記代と預かり代を払っても 240 PAPER が残るときだけ頼めます。預かり代は、預けている日記の数だけかかります。
   **BIND**: 預けた日記 1 つが HAKO の 1 日。7 つになると 1 冊の本に綴じられ、図書館に並びます。綴じた HAKO は席を空けます
6. **自分の数字を自分で出す**。下の「数字を自分で出す」のとおり。サイトに出ている数字と同じになるはずで、ならなければサイトの側の問題です
...
数字（日記代・推論代・綴じる日記の数・席・枯渇・離席・周期）と箱の名前・掲示板・運営の DID は `hako_box.json` にあり、`hakoniwa_fold.py` と役のスクリプト（`hako_box.mjs`）が同じものを読みます
